//! Benchmarks for IPv4 and IPV6 dissections.
//!
use criterion::{criterion_group, criterion_main, Criterion};

use scalpel::layers::register_defaults;
use scalpel::packet::Packet;
use scalpel::types::ENCAP_TYPE_ETH;

pub fn new_ipv6_packet_from_bytes(c: &mut Criterion) {
    let bytes = hex::decode("000573a007d168a3c4f949f686dd600000000020064020010470e5bfdead49572174e82c48872607f8b0400c0c03000000000000001af9c7001903a088300000000080022000da4700000204058c0103030801010402").unwrap();
    let _ = register_defaults();
    c.bench_function("Parse_Ethernet_IPv6", |b| {
        b.iter(|| Packet::from_bytes(&bytes, ENCAP_TYPE_ETH))
    });
}

pub fn new_ipv4_packet_from_bytes(c: &mut Criterion) {
    let _ = register_defaults();

    let bytes = hex::decode("").unwrap();

    c.bench_function("Parse_Ethernet_IPv4", |b| {
        b.iter(|| Packet::from_bytes(&bytes, ENCAP_TYPE_ETH))
    });
}

criterion_group!(
    ip_dissect,
    new_ipv6_packet_from_bytes,
    new_ipv4_packet_from_bytes
);
criterion_main!(ip_dissect);
